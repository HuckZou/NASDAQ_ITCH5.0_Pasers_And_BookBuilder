...
//g++ -std=c++11 -pthread parallelDataParser.cpp -o output
...
//function that find the starting point of a message 
...
//===========================================================//
//Data that passed in to each thread
struct ThreadData {
	int threadID;
...
//thread helper function. this specifies the task done by each thread.
void* parseThreadHelper(void *v_threadData) {
	FMap decoder;
	ThreadData *threadData;
	threadData = (ThreadData *) v_threadData;
	std::string inputFileName= "../../week1/data";
...
int main() {
	...
	// std::cout<<"reached here "<<__LINE__<<std::endl;
	...
	for(int i = 0; i < NUM_THREADS; i++) {
		...
		int rc = pthread_create(&threadIDs[i], NULL, parseThreadHelper, &threadData[i]);
		// std::cout<<"reached here "<<__LINE__<<std::endl;
	     if (rc){
